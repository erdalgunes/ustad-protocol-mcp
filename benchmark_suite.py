...
class UstadProtocolBenchmarks:
    ...
    async def run_single_benchmark(self, test: BenchmarkTest, ustad_function) -> BenchmarkResult:
        ...
        try:
            ...
            performance_data = {
                "total_latency_ms": result.get("metadata", {}).get(
            ...
            return BenchmarkResult(
                ...
                criteria_results={},
                execution_time_ms=(time.time() - start_time) * 1000,
                notes=f"Test failed with error: {e!s}",
            )
    ...
    async def run_full_benchmark_suite(self, ustad_function) -> dict[str, Any]:
        ...
        for test in self.benchmark_tests:
            ...
            print(
                f"   {status} - Score: {result.score:.1f}/100 - Time: {result.execution_time_ms:.0f}ms"
            )

            if not result.passed:
                failed_criteria = [
                    k for k, (_, _, passed) in result.criteria_results.items() if not passed
                ]
        ...
        # Category averages
        category_averages = {
            category: statistics.mean(scores) for category, scores in category_scores.items()
        }
    ...
    def _generate_benchmark_recommendations(
        self, results: list[BenchmarkResult], category_averages: dict[str, float]
    ) -> list[str]:
        ...
        if overall_score < 70:
            recommendations.append(
                "🚨 Critical: Overall benchmark performance below acceptable threshold (70%). Comprehensive review needed."
            )
        elif overall_score < 80:
            recommendations.append(
                "⚠️ Warning: Benchmark performance has room for improvement. Focus on failing test categories."
            )
        else:
            recommendations.append(
                "✅ Good: Benchmark performance meets standards. Continue optimizing for excellence."
            )
        ...
        for category, avg_score in category_averages.items():
            if avg_score < 70:
                recommendations.append(
                    f"🔧 Critical issue in {category.replace('_', ' ')}: Score {avg_score:.1f} needs immediate attention"
                )
            elif avg_score < 80:
                recommendations.append(
                    f"📈 Improve {category.replace('_', ' ')}: Score {avg_score:.1f} has optimization potential"
                )
        ...
        if slow_tests:
            recommendations.append(
                f"⚡ Performance optimization needed: {len(slow_tests)} tests exceeded 3-minute response time"
            )
        ...
        if failed_tests:
            ...
            recommendations.append(
                f"🎯 Focus improvement efforts on: {', '.join(failed_categories)}"
            )
    ...
    def print_benchmark_report(self, report: dict[str, Any]):
        ...
        print(
            f"🎯 Pass Rate: {summary['pass_rate']:.1f}% ({summary['passed_tests']}/{summary['total_tests']} tests)"
        )
        ...
        if failed_tests:
            ...
            for test in failed_tests:
                ...
                failed_criteria = [
                    k for k, (_, _, passed) in test["criteria_results"].items() if not passed
                ]
                if failed_criteria:
...
async def mock_ustad_think(
    problem: str, context: str = "", num_thoughts: int = 6
) -> dict[str, Any]:
    ...
    return {
        ...
        "best_insights": [
            ...
            "Systematic evaluation indicates balanced solution is optimal",
        ],
