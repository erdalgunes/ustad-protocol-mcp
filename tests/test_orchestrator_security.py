...
class TestCostTracker:
    ...
    async def test_warning_threshold(self):
        ...
        config = CostConfig(
            max_cost_per_request=0.10, max_cost_per_hour=0.10, warning_threshold=0.80
        )
        costs = ModelCosts()
        tracker = CostTracker(config, costs)
...
class TestSecureOrchestrator:
    ...
    async def test_respects_rate_limits(self):
        ...
        mock_orchestrator = MagicMock()
        mock_orchestrator.think_with_scaffolding = AsyncMock(return_value={"result": "success"})
        config = RateLimitConfig(requests_per_minute=1)
    ...
    async def test_tracks_costs(self):
        ...
        secure = SecureOrchestrator(mock_orchestrator, cost_config=cost_config)
