...
class BotEngine:
    ...
    async def generate_thoughts_async(
        ...
        """
        tasks = [
            self._generate_single_thought_async(prompt, context, i)
            for i in range(self.batch_size)
        ]
    ...
    def _generate_single_thought(
        ...
        strategies = [
            ...
            "tactical", "strategic", "endgame-focused",
            "development", "control", "sacrifice", "simplify"
        ]
...
class ThoughtScorer:
    ...
    def __init__(
        ...
        scoring_fn: Optional[Callable[[Thought, str], float]] = None,
    ):
        """Initialize scorer.
