...
class OpenAIBatchOfThought:
    ...
    async def generate_thought(
        ...
        # Prepare the prompt
        prompt = self.prompts[perspective].format(problem=problem, context=context)
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert thinker providing focused insights.",
                    },
            ...
            return Thought(
                perspective=perspective.value,
                content=f"Error generating {perspective.value} perspective: {e!s}",
                confidence=0.0,
                tokens_used=0,
                latency_ms=int((time.time() - start_time) * 1000),
                cost=0.0,
    ...
    def _estimate_confidence(self, content: str, perspective: PerspectiveType) -> float:
        ...
        if (
            perspective == PerspectiveType.EMPIRICAL
            and "data" in content.lower()
            or perspective == PerspectiveType.PRACTICAL
            and "step" in content.lower()
        ):
            confidence += 0.05
    ...
    async def think(
        ...
        tasks = [
            self.generate_thought(perspective, problem, context) for perspective in perspectives
        ]
    ...
    def _generate_consensus(self, thoughts: list[Thought]) -> str:
        ...
        if len(high_confidence) >= len(thoughts) * 0.6:
            return (
                f"Strong consensus across {len(high_confidence)} perspectives with high confidence"
            )
        if len(high_confidence) >= len(thoughts) * 0.3:
...
class OpenAIMCPServer:
    ...
    async def think(self, problem: str, context: str = "", num_perspectives: int = 8) -> str:
        ...
        result["usage_stats"] = {
            ...
            "average_cost_per_call": f"${self.total_cost / self.total_calls:.4f}"
