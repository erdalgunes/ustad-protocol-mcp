...
class PerfectBotMCPServer:
    ...
    def _register_tools(self):
        ...
        @self.server.tool()
        async def ustad_think(problem: str, context: str = "") -> str:
            ...
            try:
                ...
                self.history.append(
                    {"timestamp": datetime.now().isoformat(), "problem": problem, "result": result}
                )
        ...
        async def deep_analysis(
            problem: str, context: str = "", depth: str = "comprehensive"
        ) -> str:
            ...
            try:
                ...
                analysis = {
                    ...
                    "alternative_solutions": result["thoughts"][1:3]
        ...
        async def compare_solutions(
            problem: str, solutions: list[str], criteria: dict[str, float] | None = None
        ) -> str:
            ...
            try:
                ...
                for solution in solutions:
                    ...
                    comparison = {
                        ...
                        "best_perspective": result["best_thought"]["perspective"]
                        if result["best_thought"]
                        else "unknown",
                        "confidence": result["best_thought"]["confidence"]
                        if result["best_thought"]
                        else 0,
                    }
                ...
                return json.dumps(
                    {
                        ...
                        "rationale": f"Highest scoring solution with {comparisons[0]['confidence']:.0%} confidence"
                        if comparisons
                        else None,
                    },
                    indent=2,
        ...
        @self.server.tool()
        async def learn_from_feedback(problem_id: str, feedback: str, rating: int) -> str:
        ...
        @self.server.tool()
        async def get_thinking_history(limit: int = 10) -> str:
                        ...
                        "history": [
                            {
                                ...
                                "best_perspective": entry["result"]["best_thought"]["perspective"]
                                if entry["result"].get("best_thought")
                                else None,
                            }
                            for entry in recent
                        ],
