...
class PieceSquareTables:
    ...
    def get_square_value(
        self, piece_type: int, square: int, color: bool, endgame: bool = False
    ) -> int:
    ...
    def evaluate_position(self, board: chess.Board) -> int:
        ...
        for square, piece in board.piece_map().items():
            pst_value = self.get_square_value(piece.piece_type, square, piece.color, endgame)
...
class MinimaxSearcher:
    ...
    def search(self, board: chess.Board) -> tuple[chess.Move | None, float]:
        ...
        if self.use_pruning:
            return self._minimax_alpha_beta(
                board, self.depth, -float("inf"), float("inf"), board.turn
            )
        return self._minimax(board, self.depth, board.turn)

    def search_with_stats(self, board: chess.Board) -> tuple[chess.Move | None, float, int]:
        """Search and return statistics."""
        move, score = self.search(board)
        return move, score, self.nodes_searched

    def _minimax(
        self, board: chess.Board, depth: int, maximizing: bool
    ...
    def _minimax_alpha_beta(
        self, board: chess.Board, depth: int, alpha: float, beta: float, maximizing: bool
    ) -> tuple[chess.Move | None, float]:
...
class BotEvaluationThread:
    ...
    def evaluate(self, board: chess.Board) -> tuple[chess.Move | None, float]:
...
class ParallelBotAnalyzer:
    ...
    def analyze(self, board: chess.Board) -> tuple[chess.Move, float, dict[str, int]]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_threads) as executor:
            ...
            for future, thread in zip(futures, self.threads, strict=False):
