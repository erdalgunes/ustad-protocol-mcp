...
class IntelligentThoughtGenerator:
    ...
    def _detect_problem_type(self, problem: str) -> str:
        ...
        if any(word in problem_lower for word in ["optimize", "improve", "reduce", "increase"]):
            ...
            return "design"
        elif any(word in problem_lower for word in ["debug", "fix", "solve", "resolve"]):
            return "troubleshooting"
        elif any(word in problem_lower for word in ["analyze", "understand", "why", "how"]):
            return "analysis"
        elif any(word in problem_lower for word in ["choose", "decide", "should", "compare"]):
            return "decision"
        elif any(word in problem_lower for word in ["plan", "strategy", "roadmap"]):
            return "planning"
        else:
            return "general"
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        stop_words = {"the", "a", "an", "and", "or", "but", "in", "on", "at", "to", "for",
                     ...
                     "been", "be", "being", "have", "has", "had", "do", "does", "did",
                     "will", "would", "should", "could", "can", "may", "might", "must",
                     "shall", "ought", "need", "want", "how", "what", "when", "where",
                     "why", "who", "which", "this", "that", "these", "those", "we", "our"}
    ...
    def _generate_content(
        ...
        if perspective == PerspectiveType.ANALYTICAL:
            ...
            content_parts.append(f"What if we {random.choice(['inverted', 'combined', 'reimagined'])} the solution?")
            ...
            content_parts.append(f"Immediate action: Start with {analysis['keywords'][0] if analysis['keywords'] else 'core functionality'}")
            ...
            content_parts.append(f"Measure: {analysis['keywords'][0] if analysis['keywords'] else 'key metrics'}")
...
class PerfectBatchOfThought:
    ...
    def think(
        ...
        problem: str,
        context: str = "",
        custom_perspectives: List[PerspectiveType] = None
    ) -> Dict[str, Any]:
    ...
    def _generate_summary(self, thoughts: List[Thought], problem: str) -> str:
        ...
        summary_parts = [
            f"For the problem '{problem[:50]}...':" if len(problem) > 50 else f"For the problem '{problem}':",
        ...
        if len(thoughts) > 1:
            ...
            summary_parts.append(
                f"Alternative {alt.perspective.value} view: {alt.content[:80]}."
            )
