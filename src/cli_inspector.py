...
class CLIInspector:
    ...
    def __init__(self, server: Optional[ThoughtProcessor] = None) -> None:
    ...
    def test_error_handling(self) -> TestResult:
        ...
        if all(errors_caught):
            ...
            self._print_colored(
                f"❌ Error handling: FAILED ({passed_count}/{total_count} errors caught)",
                ColorCode.RED
            )
            return TestResult(
                test_name="error_handling",
    ...
    def view_session(self):
        ...
        if history:
            ...
            for thought in history[-5:]:  # Show last 5 thoughts
                print(f"  [{thought['thoughtNumber']}/{thought['totalThoughts']}] {thought['thought'][:50]}...")
